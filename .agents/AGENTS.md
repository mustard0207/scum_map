...
## 6. 微信小程序分享功能规范
- **规范**：在开发任何一个具有独立功能的新页面（如工具页、详情页等）时，**必须且绝不能遗漏**在页面的 JS 文件中实现原生的分享方法。
  - 必须添加 onShareAppMessage() 以支持发送给好友。
  - 必须添加 onShareTimeline() 以支持分享到朋友圈。
- **注意**：这与第 5 点的单页分享防卡死陷阱相辅相成。如果没有配置这两个方法，用户将无法主动分享你的工具页面。

## 7. 微信小程序内容安全审查机制（搜索框禁忌）
- **坑点**：微信小程序对允许用户自由输入文本的组件（如 `<input>` 搜索框、留言板）极其敏感。一旦被巡查脚本发现页面中包含可以输入文本的功能，且开发者**没有在后端接入并调用微信官方的「文字内容安全 API (msgSecCheck)」**，小程序将面临极大概率的审核驳回，甚至上线后被直接封禁功能。
- **解决方案与规范**：
  - **原则上禁止添加自由文本输入框**：除非极其必要且后端支持到位，我们在开发任何工具页（如字典检索、物品过滤）时，必须通过原生交互组件替代文本搜索功能。
  - **替代方案推荐**：
    - 使用原生 `<picker>`（选择器滚轮）让用户进行精确过滤。
    - 使用固定分类的标签页（Tabs / Tags）进行网格筛选。
    - 使用多维维度的点击排序（Sort Bar）进行上下整理。
  - 此规范在《武器数据手册》工具的重构中得到了完美印证（我们果断删除了搜索框，并用口径 Picker 代替）。

## 8. Windows/PowerShell 终端命令拼接规范
- **坑点**：当在 Windows 系统的 PowerShell（如默认的 5.1 版本）中运行命令时，使用类 Linux 的 `&&` 运算符拼接命令（如 `git add . && git commit`）会导致语法错误，报出 `FullyQualifiedErrorId : InvalidEndOfLine`。
- **解决方案与规范**：
  - **禁止在拼接命令时使用 `&&`**：应当使用 `;` 作为命令分隔符（例如：`git add . ; git commit`），或者直接拆分成单条命令依次执行。
  - 在编写和提议终端命令时，注意当前运行系统的环境，严格遵守此规范以防命令执行报错。