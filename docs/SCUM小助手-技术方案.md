...
> 版本：v7.4 (Z4瓦片分层加载) | 更新日期：2026-06-25
...
| ~~**瓦片加载**~~ | ~~分层加载不同缩放级别的瓦片~~ → **已在 v7.4 实现（Z2+Z4 分层加载）** |
| **Z6 瓦片** | 扩展 Z6（16×16）特写瓦片，极限放大时完全清晰（需额外分包） |
...
- **当前使用**：Z2+Z4 分层加载
  - Z2 概览图（1280x1280，306KB）— 主包，scale < 1.5 时使用
  - Z4 中景图（4×4 网格，每块 640x640，共 1.45MB）— packageMap 分包，scale ≥ 1.5 时使用
...
### 2.3 瓦片分层加载（v7.4 已实现 Z2+Z4）
| 梯队 | 原始层级 | 处理方式 | 物理尺寸 | 瓦片数 | 状态 |
|------|---------|---------|---------|-------|------|
| **概览图** | Z2 (1x1) | 无损保留 | 1280x1280 | 1张 | ✅ 已实现 |
| **中景图** | Z4 (4x4) | JPG Q=75 | 640x640 | 16张 | ✅ 已实现 |
| **特写图** | Z6 (16x16)| JPG Q=50 | 320x320 | 256张 | 🔲 待实现（需额外分包） |

**分层切换逻辑**：
- `scale < 1.5` → 使用 Z2 单图
- `scale ≥ 1.5` → 使用 Z4 网格（4×4，仅渲染视口内可见瓦片）
...
### 7.3 信息窗口
- 点击标记后显示信息窗口（宽度 80%）
- 标题区：`◆` 图标 + 标记名称（无名时显示"未命名标记"）+ 副标题"标记 #N" + 关闭按钮
- 坐标区：标签行（经度/纬度，灰色小字）+ 数值行（琥珀色大字等宽），两列对齐
- 操作区：三个等宽按钮 — `🗑 删除` / `📋 复制` / `↗ 分享`
...
- **筛选**（☰）：二期实现分类筛选，当前点击提示"即将开放"（灰化显示）

### 7.5 回到全局视角
地图左下角浮动圆形按钮（⊙），半透明风格，点击重置地图到初始缩放和位置，**不清除标记**。
...
│   └── tiles/
│       └── 2/
│           └── 0_0.jpg          # Z2 概览图（306KB，主包中）
│
├── pages/                       # 主包：功能入口
│   ├── index/
│   └── about/
│
├── packageMap/                  # 地图分包（~1.5MB）
│   ├── assets/
│   │   └── tiles/
│   │       └── 4/               # Z4 瓦片（4×4=16张 JPG，共 1.45MB）
│   │           ├── 0_0.jpg ~ 3_3.jpg
│   ├── pages/
│   │   └── map/
│   │       ├── map.js           # 地图页面逻辑
...
| # | 检查项 | 状态 | 说明 |
|---|--------|------|------|
| 1 | 代码包不超过 1.5M | ✅ | 主包约 435KB，packageMap 约 1.5MB |
| 2 | 引用插件不超过 200K | ✅ | 无插件引用 |
| 3 | 图片音频不超过 200K | ⚠️ | Z2 底图 306KB 超限；Z4 瓦片均 < 200KB ✅ |
...
| 地图滑出可视区域 | 左下角浮动"回到全局视角"按钮（⊙），一键重置缩放和位置 |
| 包体审核超限 | 主包约 435KB，packageMap 约 1.5MB，极度安全 |
| Z2 底图超过 200KB | `0_0.jpg`（306KB）超过微信 200KB 图片限制；Z4 瓦片均 < 200KB |
...
| Z4 瓦片加载失败 | Z2 底图始终渲染作为兜底；binderror 回调记录错误 |
| Z4 激活时 Z2 浪费内存 | MVP 可接受（~6MB）；后续优化：Z4 全部 load 后隐藏 Z2 |
