...
// ================================================================
// 瓦片 CDN 源切换开关
//   true  = 使用微信公众号永久素材库 (mmbiz.qpic.cn)
//   false = 使用 GitHub + jsDelivr CDN (cdn.jsdelivr.net)
// ================================================================
const USE_WECHAT_CDN = true

// ================================================================
// 故障转移调试开关
//   true  = 模拟微信素材库加载失败，验证能否自动切到 jsDelivr
//   注意：仅 USE_WECHAT_CDN=true 时有效
// ================================================================
const SIMULATE_FAILOVER = false

// 微信公众号素材库 URL 映射表（仅 USE_WECHAT_CDN=true 时加载）
let WECHAT_TILE_URLS = {}
if (USE_WECHAT_CDN) {
  try {
    WECHAT_TILE_URLS = require('./wechat-tile-urls.js')
  } catch (e) {
    console.warn('[WeChatCDN] URL 映射加载失败，回退 jsDelivr:', e)
  }
}
...
Component({
  ...
  methods: {
    ...
    _initTileCache() {
      ...
      this._cachedTileExt = {}  // {level_col_row → 'webp'|'jpg'}
      // 扫描 Z3、Z4、Z6 缓存目录
      ;[3, 4, 6].forEach(level => {
        ...
        try {
          ...
          // 按 tile key 分组，检测重复缓存
          const found = {}
          files.forEach(file => {
            const match = file.match(/^(\d+_\d+)\.(webp|jpg)$/)
            if (match) {
              const key = `${level}_${match[1]}`
              if (!found[key]) found[key] = []
              found[key].push({ file, ext: match[2] })
            }
          })
          // 注册缓存 & 清理旧格式副本
          const preferredExt = USE_WECHAT_CDN ? 'jpg' : 'webp'
          for (const [key, entries] of Object.entries(found)) {
            if (entries.length > 1) {
              // 同张瓦片有两份缓存，删掉不匹配当前格式的
              for (const entry of entries) {
                if (entry.ext !== preferredExt) {
                  try { fs.unlinkSync(cacheDir + entry.file) } catch (e) {}
                }
              }
            }
            // 注册剩下的（优先用当前格式）
            const keep = entries.find(e => e.ext === preferredExt) || entries[0]
            this._cachedTiles.add(key)
            this._cachedTileExt[key] = keep.ext
          }
        } catch (e) {
          // 该层级缓存目录不存在
        }
     ...
     * @param {number} col,row - 瓦片坐标
     * @param {boolean} preferNetwork - true=跳过缓存检查，直取网络 URL（用于缓存下载）
     * @returns {string} 图片 URL
     */
    _resolveTileUrl(level, col, row, preferNetwork = false) {
      const cfg = TILE_LEVELS[level]
      if (level === 2) return cfg.srcPrefix + '0_0.jpg'

      const tileKey = `${level}_${col}_${row}`

      // 优先本地缓存（下载缓存时跳过）
      if (!preferNetwork && this._isTileCached(level, col, row)) {
        const ext = this._cachedTileExt[tileKey] || 'webp'
        return `${wx.env.USER_DATA_PATH}/${cfg.cacheDir}${col}_${row}.${ext}`
      }

      // 故障转移：该瓦片之前从主源加载失败 → 切到备用源
      if (this._tileFailedSet && this._tileFailedSet.has(tileKey)) {
        const fallback = USE_WECHAT_CDN
          ? cfg.networkUrl.replace('{x}', col).replace('{y}', row)
          : WECHAT_TILE_URLS[tileKey]
        if (fallback) return fallback
      }

      // 主源
      if (USE_WECHAT_CDN) {
        let wechatUrl = WECHAT_TILE_URLS[tileKey]
        if (wechatUrl) {
          // 调试模式：让 Z4 瓦片"假装失败"，触发故障转移
          if (SIMULATE_FAILOVER && level === 4) {
            wechatUrl = wechatUrl.replace('mmbiz.qpic.cn', 'invalid.example.com')
          }
          return wechatUrl
        }
        console.warn(`[TileCDN] ${tileKey} 无微信素材映射，走 jsDelivr`)
      }
      return cfg.networkUrl.replace('{x}', col).replace('{y}', row)
    },

    /** 获取瓦片 src（缓存优先 → 主源 → 故障转移） */
    _getTileSrc(level, col, row) {
      return this._resolveTileUrl(level, col, row)
    },

    /** 瓦片从网络加载成功后，保存到本地缓存（含故障转移感知） */
    _saveTileToCache(level, col, row) {
      const cfg = TILE_LEVELS[level]
      const tileKey = `${level}_${col}_${row}`
      // 用 preferNetwork=true 获取实际加载成功的网络 URL
      const url = this._resolveTileUrl(level, col, row, true)
      if (!url) return

      // 根据实际来源确定缓存扩展名
      const isWeChatUrl = url.includes('mmbiz.qpic.cn')
      const ext = isWeChatUrl ? 'jpg' : 'webp'
      const cacheDir = `${wx.env.USER_DATA_PATH}/${cfg.cacheDir}`
      const cachePath = `${cacheDir}${col}_${row}.${ext}`
        ...
        success: (res) => {
          if (res.statusCode === 200) {
            ...
            fs.saveFile({
              ...
              filePath: cachePath,
              success: () => {
                this._cachedTiles.add(tileKey)
                this._cachedTileExt[tileKey] = ext
              },
    ...
    onTileError(e) {
      ...
      console.error(`[TileError] ${id} ${e.detail?.errMsg}`)

      // 故障转移：瓦片加载失败时切到备用源重试
      if (!id || id.startsWith('tile_2')) return
      const parts = id.split('_')  // tile_{level}_{row}_{col}
      const level = parseInt(parts[1])
      const r = parseInt(parts[2])
      const c = parseInt(parts[3])
      const tileKey = `${level}_${c}_${r}`

      // 避免重复 failover（备用源再失败就不再重试了）
      if (this._tileFailedSet.has(tileKey)) return
      this._tileFailedSet.add(tileKey)

      const from = USE_WECHAT_CDN ? '📱微信素材库' : '🌐jsDelivr'
      const to = USE_WECHAT_CDN ? '🌐jsDelivr' : '📱微信素材库'
      console.warn(`[TileFailover] ${tileKey} 从 ${from} 加载失败，切换到 ${to} 重试`)

      // 使瓦片缓存失效，强制 _computeTileData 重新生成 URL
      this._tileIds = ''
      this._refreshOverlayAnim()
    },
